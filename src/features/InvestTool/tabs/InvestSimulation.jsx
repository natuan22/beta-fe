...
const InvestSimulation = () => {
  ...
  const totalCategory_1 = Math.ceil(
    arrCode.reduce((total, item) => total + item.category_1, 0)
  );
  const totalCategory_2 = Math.ceil(
    arrCode.reduce((total, item) => total + item.category_2, 0)
  );
  const totalCategory_3 = Math.ceil(
    arrCode.reduce((total, item) => total + item.category_3, 0)
  );
                      ...
                      {arrCode?.length > 0 ? (
                        <>
                          ...
                          <tr>
                            ...
                            <td
                              className={`border border-solid border-[#9E9E9E] text-center ${
                                totalCategory_1 === 100
                                  ? "text-[#0BFFC4]"
                                  : "text-red-500"
                              } p-2 font-bold`}
            ...
            <button
              ...
              className={`${
                ...
                totalCategory_3 === 100
                  ? "cursor-pointer"
                  : "cursor-not-allowed"
              } w-[188px] h-[32px] bg-[#9E9E9E] rounded-[10px] text-[15px] text-center uppercase font-bold grid place-items-center `}
              onClick={callApi}
            >
              Kiểm thử
            </button>
