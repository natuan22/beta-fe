...
const SignalBoard = ({ data, yourSignalWarnings }) => {
  ...
  const filterDataBySignalWarnings = async (data, yourSignalWarnings) => {
    const validCodes = await prepareValidCodes(
        ...
        .filter((dataItem) => {
          ...
          const isValidScope = (() => {
            ...
            if (
              ...
              return true;
            if (value.scope.includes("watchlists"))
              return value.codes.includes(dataItem.code);
          ...
          return (
            isValidKey && isValidScope && isValidMarketCap && isValidLiquidity
          );
        })
  ...
  const columns = [
    {
      ...
      render: (_, record) => {
        return (
          <div
            className="text-black text-center text-lg font-semibold no-underline cursor-pointer hover:text-[#0164F8]"
  ...
  return (
    <div className="table-antd-betasmart mt-4">
      <Table
        ...
        scroll={
          dataRender.length * rowHeight > maxHeight ? { y: maxHeight } : {}
        }
        pagination={false}
      />
    </div>
