...
import { fetchDataInternationalBalanceOfPayments } from "../../thunk";

const InternationalBalanceOfPayments = () => {
  const dispatch = useDispatch();
  const { dataInternationalBalanceOfPayments } = useSelector(
    (state) => state.macro
  );
  ...
  useEffect(() => {
    dispatch(fetchDataInternationalBalanceOfPayments);
  }, [dispatch]);

  useEffect(() => {
    if (dataInternationalBalanceOfPayments?.length > 0) {
      setLoading(false);
      const modifiedArray = dataInternationalBalanceOfPayments.map((item) => {
  ...
  return (
    <div>
      {dataInternationalBalanceOfPayments?.length > 0 ? (
...
export default InternationalBalanceOfPayments;
