    ...
    if (dataPerCPIMonth?.length > 0) {
      ...
      const modifiedArray = dataPerCPIMonth.map((item) => {
        const modifiedName = `${item.name
          .replace("Tăng trưởng CPI CPI :", "")
          .replace("MoM (%)", "")} - ${item.date.slice(0, 4)}`;
                ...
                <tbody>
                  {!loading ? (
                    ...
                    dataTb.map((item) => (
                      ...
                      >
                        <th
                          className={`sticky left-0 dark:bg-[#151924] bg-gray-100 text-left align-middle whitespace-nowrap px-1 py-[26px] text-sm dark:text-white text-black`}
                        >
                          {item.name}
                        </th>
                        {item.values.map((value, index) => (
                          <td
                            key={index}
                            className={`text-sm text-center align-middle whitespace-nowrap px-1 py-[26px] font-semibold dark:text-white text-black`}
                          >
