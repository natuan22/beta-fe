...
import {
  getColor,
  getColorWithValueReference,
} from "../../../Chart/utils/utils";
...
const ForeignInvestIndex = () => {
  ...
  useEffect(() => {
    if (dataForeignInvestIndex) {
      const dataWithKey =
        Array.isArray(dataForeignInvestIndex) &&
        dataForeignInvestIndex?.map((item, index) => ({
          ...item,
          key: index,
        }));
      setData(dataWithKey);
  ...
  const mappedData =
    Array.isArray(dataForeignInvestIndex) &&
    dataForeignInvestIndex?.map((item) => ({
      text: item.name,
      value: item.name,
    }));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <p className={`${colorcm} text-center font-semibold`}>
            {record.cm.toFixed(2)}
          </p>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const calculatedValue = record.cm_usd_pre
          ? ((record.cm_usd - record.cm_usd_pre) / record.cm_usd_pre) * 100
          : 0;
        const colorcmUsdPer = getColor(calculatedValue);
        return (
          <p className={`${colorcmUsdPer} text-center font-semibold`}>
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <p className={`${colortv} text-center font-semibold`}>
            {record.tv.toFixed(2)}
          </p>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        // Thực hiện tính toán dựa trên dữ liệu và trả về giá trị tùy chỉnh
        const calculatedValue = record.tv_usd_pre
          ? ((record.tv_usd - record.tv_usd_pre) / record.tv_usd_pre) * 100
          : 0;
        const colortvUsdPer = getColor(calculatedValue);
        return (
          <p className={`${colortvUsdPer} text-center font-semibold`}>
            {calculatedValue.toFixed(2)}%
          </p>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <p className={`${colorgv} text-center font-semibold`}>
            {record.gv.toFixed(2)}
          </p>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        // Thực hiện tính toán dựa trên dữ liệu và trả về giá trị tùy chỉnh
        const calculatedValue = record.gv_usd_pre
          ? ((record.gv_usd - record.gv_usd_pre) / record.gv_usd_pre) * 100
          : 0;
        const colorgvUsdPer = getColor(calculatedValue);
        return (
          <p className={`${colorgvUsdPer} text-center font-semibold`}>
            {calculatedValue.toFixed(2)}%
          </p>
        );
      },
