...
const IndexIndustrialProduction = () => {
  ...
  const { dataTableIndexIndustrialProduction } = useSelector(
    (state) => state.macro,
  );
      ...
      <section className="bg-blueGray-50 pt-1.5">
        <div className="w-full">
          <div className="relative flex flex-col min-w-0 break-words bg-transparent w-full rounded">
            <div className="block xxs:w-[295px] xs:w-[350px] sm:w-[400px] md:w-[670px] lg:w-[897px] xl:w-full scrollbar-thin scrollbar-thumb-[#0050AD] dark:scrollbar-track-[#151924] scrollbar-track-transparent overflow-x-scroll bg-transparent h-[300px]">
                  ...
                  {!loading ? (
                    ...
                    dataTb.map((item) => {
                      return (
                        ...
                        >
                          <th
                            className={`sticky left-0 dark:bg-[#151924] bg-gray-100 text-left align-middle whitespace-nowrap px-1 py-[18px] cursor-pointer text-sm dark:text-white text-black dark:hover:bg-gray-800 hover:bg-gray-300 duration-500`}
                            onClick={() => {
                              dispatch(
                                fetchDataIndexIndustrialProduction(
                                  hashtb[item.name],
                          ...
                          {item.values.map((value, index) => (
                            <td
                              key={index}
                              className={`text-sm text-center align-middle whitespace-nowrap px-1 py-[18px] font-semibold dark:text-white text-black`}
                            >
