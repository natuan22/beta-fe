...
const EstimatedValueBondsDueDate = () => {
    ...
    const { dataEstimatedValueBondsDueDate } = useSelector(state => state.macro)
    ...
    useEffect(() => {
        dispatch(fetchDataEstimatedValueBondsDueDate)
    }, [dispatch]);

    useEffect(() => {
        if (dataEstimatedValueBondsDueDate?.length > 0) {
            const modifiedArray = dataEstimatedValueBondsDueDate.map(item => {
    ...
    return (
        <>
            {dataEstimatedValueBondsDueDate?.length > 0 ? (
...
export default EstimatedValueBondsDueDate