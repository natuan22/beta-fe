...
const TransactionData = ({ stock, from, to }) => {
  ...
  useEffect(() => {
    if (dataTransactionData) {
      const dataWithKey =
        Array.isArray(dataTransactionData) &&
        dataTransactionData?.map((item, index) => ({
          ...item,
          key: index,
        }));
      setData(dataWithKey);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <p className={`dark:text-white text-black text-center font-semibold`}>
            {record.closePrice === 0 ? "-" : record.closePrice}
          </p>
        );
      },
    },
      ...
      children: [
        {
          ...
          render: (_, record) => {
            return (
              <p
                className={`dark:text-white text-black text-center font-semibold`}
              >
                {record.omVol === 0 ? "-" : formatNumberCurrency(record.omVol)}
              </p>
            );
          },
        },
          ...
          render: (_, record) => {
            return (
              ...
              >
                {record.omVal === 0
                  ? "-"
                  : formatNumberCurrency(record.omVal / 1000000000)}
              </p>
            );
          },
        },
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <p className={`dark:text-white text-black text-center font-semibold`}>
            {record.highPrice === 0 ? "-" : record.highPrice}
          </p>
        );
      },
    },
    {
      ...
      render: (_, record) => {
        return (
          <p className={`dark:text-white text-black text-center font-semibold`}>
            {record.lowPrice === 0 ? "-" : record.lowPrice}
          </p>
        );
      },
    },
