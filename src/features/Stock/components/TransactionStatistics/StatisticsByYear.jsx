      ...
      {dataStatisticsByYear?.length > 0 ? (
        <div className='mt-4 xl:w-full lg:w-[411px] md:w-full sm:w-full xs:w-full xxs:w-full'>
          <div className='bg-[#1D5F8D] w-full h-[44px] flex justify-evenly items-center'>
            <button className={` ${currentYearIndex === dataStatisticsByYear.length - 1 ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'} bg-transparent border-0 text-xl text-[#C3FFB4] `}>
              <BiSolidLeftArrow onClick={handlePreYear} />
            </button>
            <span className='date text-white'>{currentDate}</span>
            <button className={` ${currentYearIndex === 0 ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'} bg-transparent border-0 text-xl text-[#C3FFB4] `}>
