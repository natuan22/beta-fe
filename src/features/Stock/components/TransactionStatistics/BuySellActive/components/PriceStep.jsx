...
const formatNumberCurrency = (number) => {
  ...
  const [integerPart, decimalPart] = absoluteValue.includes(".")
    ? absoluteValue.split(".")
    : [absoluteValue, null];
  ...
  if (decimalPart !== null && Math.abs(number) < 1000) {
    formattedNumber = `${
      isNegative ? "-" : ""
    }${formattedIntegerPart},${decimalPart.padEnd(1, "0")}`;
  } else {
...
const PriceStep = ({ data }) => {
  ...
  const handleMouseOver = useCallback(function (isAtc) {
    ...
    if (this && this.chart) {
      ...
      if (this.chart.series[atoSeriesIndex]) {
        this.chart.series[atoSeriesIndex].update({
  ...
  const chartData = useMemo(() => {
    if (!data?.matchPriceGroups?.length)
    ...
    const createSeriesData = (key, isAtc) => ({
      ...
      events: {
        mouseOver: function () {
          handleMouseOver.call(this, isAtc);
        },
        mouseOut: function () {
          handleMouseOut.call(this, isAtc);
        },
      },
    });
    ...
    const seriesVal = [
      ...
      {
        name: "Bán chủ động",
        data: sortedData.map(({ totalValSell }) => ({
          y: totalValSell / 1_000_000_000,
          color: "red",
        })),
        color: "red",
      },
      {
        name: "Mua chủ động",
        data: sortedData.map(({ totalValBuy }) => ({
          y: totalValBuy / 1_000_000_000,
          color: "green",
        })),
        color: "green",
      },
    ...
    const seriesVol = [
      ...
      {
        name: "Bán chủ động",
        data: sortedData.map(({ totalVolSell }) => ({
          y: totalVolSell / 1_000,
          color: "red",
        })),
        color: "red",
      },
      {
        name: "Mua chủ động",
        data: sortedData.map(({ totalVolBuy }) => ({
          y: totalVolBuy / 1_000,
          color: "green",
        })),
        color: "green",
  ...
  useEffect(() => {
    if (
      ...
      (state.categories !== chartData.categories ||
        state.series !== chartData.series)
    ) {
      setState((prevState) => ({
        ...prevState,
  ...
  const addMouseWheelEvent = (chart) => {
    ...
    chart.container.addEventListener("wheel", (e) => {
      ...
      newMin = Math.max(
        extremes.dataMin,
        Math.min(newMin, extremes.dataMax - (extremes.max - extremes.min)),
      );
      newMax = Math.min(
        extremes.dataMax,
        Math.max(newMax, extremes.dataMin + (extremes.max - extremes.min)),
      );
  ...
  const options = useMemo(() => {
    return {
      ...
      xAxis: {
        ...
        labels: {
          style: { fontSize: "11px", fontWeight: "bold", color },
          formatter: function () {
            const color =
              this.value === data?.prevClosePrice
                ? "#eab308"
                : this.value < data?.prevClosePrice
                  ? "#ef4444"
                  : "#22c55e";
      ...
      yAxis: {
        title: {
          text: state.isChartValue ? "Tỷ VNĐ" : "Nghìn cổ phiếu (CP)",
          style: { fontSize: "9px", fontWeight: "bold", color },
        },
        max: maxDatayAxis,
        gridLineWidth: 0,
        labels: { style: { fontSize: "9px", fontWeight: "bold", color } },
        stackLabels: {
          enabled: true,
          formatter: function () {
            ...
            return `<span>${formatNumberCurrency(this.total)} (${
              total > 0 ? `${formatNumberCurrency(percentage)}%` : "-"
            })</span>`;
          },
          style: {
      ...
      legend: {
        enabled: true,
        verticalAlign: "top",
        itemStyle: { fontWeight: "bold", fontSize: "11px", color },
      },
      plotOptions: {
        series: {
        ...
        bar: {
          ...
          dataLabels: {
            ...
            style: { textOutline: "1px black", fontWeight: "bold" },
          },
        },
      },
      series: state.series,
      tooltip: {
        ...
        pointFormatter: function () {
          return `<span style="color:${this.color}">●</span> ${
            ...
            state.isChartValue ? "tỷ đồng" : "nghìn CP"
          }<br/>`;
        },
      },
    };
  ...
  return (
    <div>
      {state.hasData ? (
        <div className="h-[635px] price-step">
          <div className="flex gap-4">
            <div
              ...
              className="py-1 font-bold text-[#0050AD] hover:text-[#008eecd0] cursor-pointer border-0 text-[15px] relative"
            >
        ...
        <div className="text-center pt-[53px] pr-[16px] font-semibold dark:text-white text-black">
