...
import { fetchDataTableFinancialIndicators } from '../../../thunk'

const TableFinancialIndicators = ({ queryApiBusinessFinance }) => {
    const dispatch = useDispatch()
    const { dataTableFinancialIndicators } = useSelector(state => state.stock)
    const [dates, setDates] = useState()
    const [dataTb, setDataTb] = useState()
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        dispatch(fetchDataTableFinancialIndicators(queryApiBusinessFinance.stock, queryApiBusinessFinance.order));
    }, [dispatch, queryApiBusinessFinance]);

    useEffect(() => {
        if (dataTableFinancialIndicators?.length > 0) {
            setLoading(false);
            let modifiedArray;

            if (queryApiBusinessFinance.order === '0') {
                modifiedArray = dataTableFinancialIndicators.map(item => {
...
export default TableFinancialIndicators