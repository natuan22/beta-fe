...
import { fetchDataTableBusinessReport } from '../../../thunk'

const TableBusinessReport = ({ queryApiBusinessFinance }) => {
    const dispatch = useDispatch()
    const { dataTableBusinessReport } = useSelector(state => state.stock)
    const [dates, setDates] = useState()
    const [dataTb, setDataTb] = useState()
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        dispatch(fetchDataTableBusinessReport(queryApiBusinessFinance.stock, queryApiBusinessFinance.order));
    }, [dispatch, queryApiBusinessFinance]);

    useEffect(() => {
        if (dataTableBusinessReport?.length > 0) {
            setLoading(false);
            let modifiedArray;

            if (queryApiBusinessFinance.order === '0') {
                modifiedArray = dataTableBusinessReport.map(item => {
    ...
    }, [dataTableBusinessReport, queryApiBusinessFinance])
...
export default TableBusinessReport