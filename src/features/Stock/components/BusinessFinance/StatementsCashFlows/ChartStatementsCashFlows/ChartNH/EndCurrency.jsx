...
const EndCurrency = ({ queryApiBusinessFinance }) => {
        ...
        if (dataChartStatementsCashFlows?.length > 0) {
            ...
            if (queryApiBusinessFinance.order === '0') {
                modifiedArray = dataChartStatementsCashFlows.map(item => {
                    const modifiedName = `${item.name.toLowerCase().replace('tiền và tương đương tiền cuối kỳ', 'Tiền cuối kỳ')}`;
    ...
    const options = {
        ...
        series: Array.isArray(data) && data.slice(5, 6),
    };
