...
const InvestmentActivities = ({ queryApiBusinessFinance }) => {
        ...
        if (dataChartStatementsCashFlows?.length > 0) {
            ...
            if (queryApiBusinessFinance.order === '0') {
                modifiedArray = dataChartStatementsCashFlows.map(item => {
                    const modifiedName = `${item.name.toLowerCase().replace('lưu chuyển tiền thuần từ hoạt động đầu tư', 'Từ hoạt động đầu tư')}`;
    ...
    const options = {
        ...
        series: Array.isArray(data) && data.slice(2, 3),
    };
...
export default InvestmentActivities