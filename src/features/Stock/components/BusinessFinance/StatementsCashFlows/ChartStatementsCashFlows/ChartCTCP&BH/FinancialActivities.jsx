...
const FinancialActivities = ({ queryApiBusinessFinance }) => {
        ...
        if (dataChartStatementsCashFlows?.length > 0) {
            ...
            if (queryApiBusinessFinance.order === '0') {
                modifiedArray = dataChartStatementsCashFlows.map(item => {
                    const modifiedName = `${item.name.toLowerCase().replace('lưu chuyển tiền thuần từ hoạt động tài chính', 'Từ hoạt động tài chính')}`;
