...
import ChartFinancialIndicators from './FinancialIndicators/ChartFinancialIndicators';
import TableFinancialIndicators from './FinancialIndicators/TableFinancialIndicators';
            ...
            <div className='mt-8'>
                <div className='md:flex sm:block justify-between'>
                    <div>
                        ...
                        <button className={`statementsCashFlows-btn ml-5 ${activeBtn === 1 ? 'statementsCashFlows-active-btn' : 'statementsCashFlows-btn-2'}`}
                            onClick={() => {
                                setActiveBtn(1)
                            }}>
                            Biểu đồ
                    ...
                    <div className='flex sm:flex-row xs:flex-col xxs:flex-col md:mt-0 sm:mt-4 xs:mt-4 xxs:mt-4 justify-center items-center'>
                        <div className='ml-4'>
                            ...
                            <select
                                className={`dark:bg-[#151924] h-[41px] bg-gray-100 dark:hover:bg-gray-900 hover:bg-gray-300 ml-2 p-1 text-base dark:text-white text-black border-[#0055B6] border-[3px] outline-0`}
                                onChange={(event) => {
                        ...
                        <div className='ml-4 sm:mt-0 xs:mt-2 xxs:mt-2'>
                            ...
                            <select
                                className={`dark:bg-[#151924] h-[41px] bg-gray-100 dark:hover:bg-gray-900 hover:bg-gray-300 sm:ml-2 xs:ml-[30px] xxs:ml-[30px] p-1 text-base dark:text-white text-black border-[#0055B6] border-[3px] outline-0`}
                                onChange={(event) => {
                ...
                {activeBtn === 0 ? (
                    <div>
                        <TableFinancialIndicators queryApiBusinessFinance={queryApiBusinessFinance} />
                    </div>
                ) : (
                    <div>
                        <ChartFinancialIndicators queryApiBusinessFinance={queryApiBusinessFinance} />
                    </div>
                )}
            </div>
        </div>
