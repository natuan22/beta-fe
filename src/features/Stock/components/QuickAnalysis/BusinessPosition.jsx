...
const BusinessPosition = ({ queryApi }) => {
    const { dataBusinessPosition, dataBusinessPositionStatus } = useSelector(state => state.stock)
    ...
    if (dataBusinessPositionStatus === 400 || dataBusinessPosition.length === 0) {
        return (
            <>
                <div className='border-solid dark:border-white border-b-[1px] border-t-0 border-x-0'>
                    <span className='dark:text-white text-black font-semibold xl:w-[50%] lg:w-[35%] md:w-[40%] sm:w-[45%] flex justify-between'>Vị thế doanh nghiệp
            ...
            {dataBusinessPosition ? (
                <div>
                    <div className='grid sm:grid-cols-2 xs:grid-cols-none items-center'>
                        <ChartGauge data={dataBusinessPosition.totalStar} />
                        <div className='dark:text-white text-black text-justify'>Từ những dữ liệu được ghi nhận chúng tôi đánh giá Cổ phiếu {queryApi.stock} có yếu tố vị thế doanh nghiệp {getTextColorRating(dataBusinessPosition.totalStar).text} với mức điểm số <span className={getTextColorRating(dataBusinessPosition.totalStar).color}>{dataBusinessPosition.totalStar}/5</span>.</div>
                    </div>

                    <div>
                        <ul className='ml-[40px] mt-1'>
                            {Array.isArray(dataBusinessPosition.data) && dataBusinessPosition.data?.map((item, index) => {
                                const contentChild = (
                                    <div>
                                        <span className='text-black font-medium rounded-lg text-sm bg-white p-2'>
                                            {item.name}
