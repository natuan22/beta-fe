...
const BusinessPosition = ({ queryApi }) => {
  const { dataBusinessPosition, dataBusinessPositionStatus } = useSelector(
    (state) => state.stock
  );
        ...
        <div>
          <div className="grid sm:grid-cols-2 xs:grid-cols-none items-center">
            ...
            <div className="dark:text-white text-black text-justify">
              Từ những dữ liệu được ghi nhận chúng tôi đánh giá Cổ phiếu{" "}
              {queryApi.stock} có yếu tố vị thế doanh nghiệp{" "}
              {getTextColorRating(dataBusinessPosition.totalStar).text} với mức
              điểm số{" "}
              <span
                className={
                  getTextColorRating(dataBusinessPosition.totalStar).color
                }
              >
                {dataBusinessPosition.totalStar}/5
              </span>
              .
            </div>
          </div>
          ...
          <div>
            <ul className="ml-[40px] mt-1">
              {Array.isArray(dataBusinessPosition.data) &&
                dataBusinessPosition.data?.map((item, index) => {
                  const contentChild = (
                    <div>
                      <span className="text-black font-medium rounded-lg text-sm bg-white p-2">
                        {item.name}
