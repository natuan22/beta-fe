...
const TechnicalAnalysis = ({ queryApi }) => {
  const { dataTechnicalAnalysis, dataTechnicalAnalysisStatus } = useSelector(
    (state) => state.stock
  );
  ...
  if (
    dataTechnicalAnalysisStatus === 400 ||
    dataTechnicalAnalysis.length === 0
  ) {
    return (
      <>
      ...
      {dataTechnicalAnalysis ? (
        <div>
          <div className="grid sm:grid-cols-2 xs:grid-cols-none items-center">
            <ChartGauge data={dataTechnicalAnalysis.totalStar} />
            <div className="dark:text-white text-black text-justify">
              Với những yếu tố đánh giá về Phân tích kỹ thuật cho thấy{" "}
              {queryApi.stock} đang có tính hiệu{" "}
              {getTextColorRating(dataTechnicalAnalysis.totalStar).text} với mức
              điểm số{" "}
              <span
                className={
                  getTextColorRating(dataTechnicalAnalysis.totalStar).color
                }
              >
                {dataTechnicalAnalysis.totalStar}/5
              </span>
              .
            </div>
          </div>
          ...
          <div>
            <ul className="ml-[40px] mt-1">
              {Array.isArray(dataTechnicalAnalysis.data) &&
                dataTechnicalAnalysis.data?.map((item, index) => {
                  const contentChild = (
                    <div>
                      <span className="text-black font-medium rounded-lg text-sm bg-white p-2">
                        {item.name}
