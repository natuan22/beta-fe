...
const FinancialHealthAnalysis = ({ queryApi }) => {
  const { dataFinancialHealthAnalysis, dataFinancialHealthAnalysisStatus } =
  ...
  if (
    dataFinancialHealthAnalysisStatus === 400 ||
    dataFinancialHealthAnalysis.length === 0
  ) {
    return (
      <>
        ...
        <div>
          <div className="grid sm:grid-cols-2 xs:grid-cols-none items-center">
            ...
            <div className="dark:text-white text-black text-justify">
              ...
              {queryApi.stock} có sức khỏe tài chính{" "}
              {getTextColorRating(dataFinancialHealthAnalysis.totalStar).text}{" "}
              với mức điểm số{" "}
              <span
                className={
                  getTextColorRating(dataFinancialHealthAnalysis.totalStar)
                    .color
                }
              >
                {dataFinancialHealthAnalysis.totalStar}/5
              </span>
              .
            </div>
          </div>
          ...
          <div>
            <ul className="ml-[40px] mt-1">
              {Array.isArray(dataFinancialHealthAnalysis.data) &&
                dataFinancialHealthAnalysis.data?.map((item, index) => {
                  const contentChild = (
                    <div>
                      <span className="text-black font-medium rounded-lg text-sm bg-white p-2">
                        {item.name}
