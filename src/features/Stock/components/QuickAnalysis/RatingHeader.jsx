    ...
    <div>
      <div className="flex w-full border-solid dark:border-white border-black border-b-2 border-t-0 border-x-0 pb-1">
        <div className="dark:text-white text-black flex items-center justify-center">
          <div
            className="icon_left cursor-pointer mr-2"
            onClick={() => {
          ...
          <div
            className="icon_right cursor-pointer ml-2"
            onClick={() => {
              setIsBasicPrice(!isBasicPrice);
            }}
          >
            ...
            <tbody>
              {dataBasicPrice?.map((item, index) => (
                <tr key={index}>
                  <td
                    className={`${
                      item.name === "Tổng hợp định giá" ? "font-bold" : ""
                    } p-2`}
                  >
                    {item.name}
                  </td>
                  ...
                  >
                    {item.value == 0
                      ? "-"
                      : Math.floor(item.value).toLocaleString("vi-VN")}
                  </td>
                  <td
                    className={`${
                      item.name === "Tổng hợp định giá"
                  ...
                  >
                    {item.value_industry == 0
                      ? "-"
                      : Math.floor(item.value_industry).toLocaleString("vi-VN")}
                  </td>
                </tr>
              ))}
            </tbody>
                ...
                <tr key={index}>
                  ...
                  <td
                    className={`p-2 text-center ${
                      index === dataTechnical.length - 1
                        ? "border border-solid dark:border-white border-black border-b-2 border-t-0 border-x-0"
                        : ""
                    }`}
                  >
                    {" "}
                    {checkFormatData(item.value, item.name)}
                  </td>
                </tr>
              ))}
            </tbody>
