...
function AnalyzeReport() {
    const [activeTab, setActiveTab] = useState(localStorage.getItem('userAnalyzeReportTabCurrent'));
            ...
            <div className='flex flex-col justify-center mt-4'>
                <div className="tab-container w-auto flex justify-evenly bg-[#195070] py-1 rounded-3xl z-0 relative mt-1.5">
                    ...
                    {tabLabels.map((label, index) => (
                        <button
                            key={index}
                            className={`relative z-10 ${activeTab === index ? 'active ' : ''} w-[50%] cursor-pointer py-1 text-[14px] border-0 rounded-full font-bold text-white bg-transparent`}
