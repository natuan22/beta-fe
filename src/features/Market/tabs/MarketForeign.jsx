...
const MarketForeign = () => {
  ...
  useEffect(() => {
    if (
      location.pathname === "/thi-truong/thi-truong-quoc-te/chi-so-the-gioi"
    ) {
      setActiveNavlink(0);
    } else if (
      location.pathname === "/thi-truong/thi-truong-quoc-te/thi-truong-hang-hoa"
    ) {
      setActiveNavlink(1);
    } else if (
      location.pathname === "/thi-truong/thi-truong-quoc-te/thi-truong-tien-so"
    ) {
      setActiveNavlink(2);
    }
  }, [location.pathname]);
        ...
        <div className="relative flex justify-around mt-5 bg-[#195070] h-auto items-center rounded-full">
          ...
          {Object.entries(hashTbMarketForeign).map(([label, value], index) => (
            <NavLink
              ...
              } z-10 w-2/6 bg-transparent text-white rounded-full no-underline text-center leading-8 md:text-lg sm:text-[15px] xs:text-[13px] xxs:text-[11px]`}
            >
              {label}
            </NavLink>
          ))}
