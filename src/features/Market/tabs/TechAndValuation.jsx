    ...
    <>
      <div className='container mx-auto'>
        <div className="relative flex justify-around mt-5 bg-[#195070] h-auto items-center rounded-full">
          ...
          {Object.entries(hashTbTechAndValuation).map(([label, value], index) => (
            <NavLink
              ref={el => navRef.current[index] = el}
              to={value}
              key={index}
              onClick={() => handleActiveNav(index)}
              className={`${activeNavlink === index ? 'active' : ''} z-10 w-2/6 bg-transparent text-white rounded-full no-underline text-center leading-8 md:text-lg sm:text-[17px] xs:text-[15px] xxs:text-[13px]`}
            >
              {label}
            </NavLink>
          ))}
