...
import {
  fetchDataIndustryCashFlow,
  fetchDataRSI,
  fetchDataTopNetBuyIndustry,
} from "../../thunk";
...
const IndustryCashFlow = () => {
  ...
  useEffect(() => {
    if (dataIndustryCashFlow && dataRSI && dataTopNetBuyIndustry) {
      const newDataWithChanges =
        Array.isArray(dataRSI) &&
        dataRSI?.map((oldItem) => {
          const matchingItem =
  ...
  const dataTuDoanh =
    Array.isArray(dataTopNetBuy) &&
    dataTopNetBuy.filter((transaction) => transaction.type === 0);
  const dataKhoiNgoai =
    Array.isArray(dataTopNetBuy) &&
    dataTopNetBuy.filter((transaction) => transaction.type === 1);
  const dataCaNhan =
    Array.isArray(dataTopNetBuy) &&
    dataTopNetBuy.filter((transaction) => transaction.type === 2);
                  ...
                  {!loading ? (
                    ...
                    data.map((item, index) => {
                      ...
                      return (
                        ...
                        >
                          <th
                            className={`${color} text-left align-middle xxs:text-[10px] lg:text-sm xl:text-xs px-1 py-2.5`}
                          >
                            {item.industry}
                          </th>
                          <td
                            className={`${color} align-middle xxs:text-[10px] lg:text-sm xl:text-xs whitespace-nowrap px-1 py-2.5 font-semibold`}
                          >
                            <span className="text-left px-1.5">
                              {getIcon(item.proprietaryPerChange)}
                            </span>
                            <span className="text-right px-px">
                              {item.proprietaryPerChange &&
                                item.proprietaryPerChange.toLocaleString(
                                  "vi-VN",
                                  {
                          ...
                          <td
                            className={`${color2} align-middle xxs:text-[10px] lg:text-sm xl:text-xs whitespace-nowrap px-1 py-2 font-semibold`}
                          >
                            <span className="text-left px-1.5">
                              {getIcon(item.foreignPerChange)}
                            </span>
                            <span className="text-right px-px">
                              {item.foreignPerChange &&
                                item.foreignPerChange.toLocaleString("vi-VN", {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                              %
                          ...
                          <td
                            className={`${color3} xl:text-left lg:text-center md:text-center align-middle xxs:text-[10px] lg:text-sm xl:text-xs whitespace-nowrap px-1 py-2 font-semibold`}
                          >
                            <span className="text-left px-1.5">
                              {getIcon(item.retailPerChange)}
                            </span>
                            <span className="text-right px-px">
                              {item.retailPerChange &&
                                item.retailPerChange.toLocaleString("vi-VN", {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                              %
        ...
        <div className="flex dark:text-white text-black">
          <div className="w-3/5">
            <div className="text-[10px] py-2">
              TOP NGÀNH KHỐI NGOẠI MUA RÒNG:{" "}
              <span className="font-semibold md:inline xs:block">
                {dataTopNetBuy &&
                  dataKhoiNgoai.length > 0 &&
                  dataKhoiNgoai[0].industry}
              </span>
            </div>
            <div className="text-[10px] py-2">
              TOP NGÀNH TỰ DOANH MUA RÒNG:{" "}
              <span className="font-semibold md:inline xs:block">
                {dataTopNetBuy &&
                  dataTuDoanh.length > 0 &&
                  dataTuDoanh[0].industry}
              </span>
            </div>
            <div className="text-[10px] py-2">
              TOP NGÀNH CÁ NHÂN & TCTN MUA RÒNG:{" "}
              <span className="font-semibold md:inline xs:block">
                {dataTopNetBuy &&
                  dataCaNhan.length > 0 &&
                  dataCaNhan[0].industry}
              </span>
            </div>
          </div>
          <div className="w-2/5">
            <div className="grid grid-cols-2">
              <div className="text-[10px] py-2">
                GT MUA:{" "}
                <span className="text-green-500 font-semibold md:inline xs:block">
                  {dataTopNetBuy &&
                    dataKhoiNgoai.length > 0 &&
              ...
              <div className="text-[10px] py-2">
                GT BÁN:{" "}
                <span className="text-red-500 font-semibold md:inline xs:block">
                  {dataTopNetBuy &&
                    dataKhoiNgoai.length > 0 &&
