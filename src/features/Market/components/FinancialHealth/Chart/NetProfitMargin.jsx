...
const NetProfitMargin = () => {
    const { dataChartNetProfitMargin } = useSelector(state => state.market)
    ...
    useEffect(() => {
        if (dataChartNetProfitMargin?.length > 0) {
            const transformedData = dataChartNetProfitMargin?.map(item => {
    ...
    }, [dataChartNetProfitMargin, industryQuery])
    ...
    return (
        <div>
            ...
            {dataChartNetProfitMargin?.length ? (
