...
const TableAveragePB = () => {
    const { dataTableAveragePB } = useSelector((state) => state.market);
    const [loading, setLoading] = useState(true);
    const [data, setData] = useState([]);

    useEffect(() => {
        if (dataTableAveragePB) {
            setLoading(false);
            setData(dataTableAveragePB)
        }
    }, [dataTableAveragePB])
                        ...
                        <table className="items-center w-full border-collapse bg-transparent">
                            <thead className="sticky top-0 bg-[#1E5D8B] z-10">
                                <tr>
                                    ...
                                    <th className="text-center align-middle px-3 py-[19px] text-[13px] font-semibold text-white">
                                        Thay đổi Giá trị sổ sách 04 quý cùng kỳ (%)
                                    </th>
                                    <th className="text-center align-middle px-3 py-[19px] text-[13px] font-semibold text-white">
                                        P/B (Lần)
                                    </th>
...
export default memo(TableAveragePB)