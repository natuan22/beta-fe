...
const MarketMap = () => {
  ...
  for (const [sector, sectorData] of Object.entries(dataTreeMap)) {
    ...
    const stockPoints = Object.entries(sectorData.data)?.map(
      ([stock, value], stockIndex) => {
        const stockPoint = {
          ...
          dataLabels: {
            ...
            formatter: function () {
              return (
                ...
                this.point.name +
                "</b>: " +
                formatNumberCurrency(this.point.value)
              );
            },
            style: {
              fontSize: "11px",
        ...
        <div>
          <div className="dark:bg-[#2D303A] bg-gray-400 flex justify-around items-center rounded-full mb-2">
            <button
              ...
              className="rounded-tl-xl rounded-bl-xl md:text-[0.8rem] lg:text-[0.9rem]"
            >
              Vốn hoá
            </button>
            <button
              style={
                activeButton2 === ENUM.gtGD
                  ? { ...buttonStyle, ...activeButtonStyle }
                  : buttonStyle
              }
              onClick={() => {
              ...
              className="md:text-[0.8rem] lg:text-[0.9rem]"
            >
              Giá trị GD
            </button>
            <button
              style={
                activeButton2 === ENUM.klGD
                  ? { ...buttonStyle, ...activeButtonStyle }
                  : buttonStyle
              }
              onClick={() => {
            ...
            <button
              ...
              className="rounded-tr-xl rounded-br-xl md:text-[0.8rem] lg:text-[0.9rem]"
            >
              Giá trị NN GD
            </button>
          </div>
