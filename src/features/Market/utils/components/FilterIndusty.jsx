...
const FilterIndusty = ({ onSelectedNamesChange }) => {
  ...
  const createHotImage = (industry) => {
    if (
      dataHotIndustry?.length > 0 &&
      dataHotIndustry?.some((item) => hashTb[item.industry] === industry)
    ) {
      if (
        industry === "hangHoa" ||
        industry === "oto" ||
        industry === "truyenThong" ||
        industry === "thucPham"
      ) {
        return (
          <img
            className="relative w-[50px] h-[30px]  ml-4 md:left-[-15px] sm:left-0"
        ...
        return (
          <img
            className="relative w-[50px] h-[30px] ml-2 "
            src={`${apiUrl}/resources/icons/hot.png`}
            alt="icon"
          />
        );
