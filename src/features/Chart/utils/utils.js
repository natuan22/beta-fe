...
export function getColorText(item) {
  ...
  if (item === item.toUpperCase() && !nameArr.includes(item))
    color = "text-[#13CCC1]";
...
export function getText(item) {
  let text = "";
  if (item === 0) text = "Trung tính";
  else if (item < "0") text = "Tiêu cực";
  else text = "Tích cực";
...
export function getTextSellBuy(item) {
  let text = <span></span>;
  if (item < "0") text = <span className="text-red-500">Bán ròng</span>;
  else text = <span className="text-green-500">Mua ròng</span>;
