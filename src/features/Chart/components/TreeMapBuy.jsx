...
const TreeMapBuy = () => {
  ...
  console.log('dataTreemapBuy',dataTreemapBuy)
  const [dataTreeMap, setDataTreeMap] = useState()
  useEffect(() => {
    if (dataTreemapBuy?.length > 0) {
        const resultMap = {};
        dataTreemapBuy.forEach(item => {
            const { LV2, ticker, total_value_buy } = item;

            if (!resultMap.hasOwnProperty(LV2)) {
                resultMap[LV2] = {};
            }

            resultMap[LV2][ticker] = (total_value_buy / 1000000000).toFixed(2);
        });
...
const points = [];
    ...
    const options = {
      ...
      title: {
          text: "Khối ngoại mua ròng theo sàn",
          align: "center"
      },
  ...
  return (
    (dataTreemapBuy.length > 0 ? <HighchartsReact highcharts={Highcharts} options={options} containerProps={{ style: { height: '654px', width: '100%' } }} /> : <div><Loading /></div>)
